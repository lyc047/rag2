...
- **混合检索** — BM25 关键词 + ChromaDB 向量语义 RRF（Reciprocal Rank Fusion）加权融合，支持精确短语匹配奖励
- **流式交互** — Agent 对话和文件上传均使用 SSE 实时推送进度和 Token
- **反幻觉机制** — 归因强制 Prompt + 相似度阈值过滤 + 未记载前置检查 + 遗漏自检（非流式）
...
> **版本号规则**：前端改动（UI、样式、交互）递增第三位（1.0.x）；RAG 内核功能改进（检索、Agent、LLM）递增第二位（1.x.0）。
